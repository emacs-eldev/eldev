...
(defmacro eldev-using-global-package-archive-cache (&rest body)
  ...
  `(eldev-advised (#'url-retrieve-synchronously :around (when eldev-enable-global-package-archive-cache #'eldev--global-cache-url-retrieve-synchronously))
     ,@body))
                       ...
                       (catch 'dir
                         (dolist (archive package-archives)
                           ...
                           (when (string-prefix-p archive-url url)
                             ...
                             ;; Don't try to cache funny names, especially if it somehow
                             ;; includes a slash.
                             (when (string-match-p (rx bos (1+ (any "a-zA-Z0-9" "-" ".+_@")) eos) filename)
                               ...
                               ;; Contents of other archives also gets cached, but with
                               ;; encoded URL as directory name in case different projects
                               ;; use the same archive under different names or different
                               ;; archives under the same name.
