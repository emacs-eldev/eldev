...
<<filesets,fileset>>.  Best is to _add_ to the default value rather
than replace it.  Here is an example from Eldev’s own file `Eldev`:

[source]
----
(setf eldev-files-to-package
      (append eldev-files-to-package
              '("./bin" "!./bin/**/*.in" "!./bin/**/*.part")))
----

Here we instruct package builder to additionally copy all files from
subdirectory `bin`, except those with names ending in `.in` or
`.part`.  If you distribute your package through MELPA, you will
unfortunately need to repeat these instructions in MELPA recipe.
