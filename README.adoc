...
this: at least {uri-nix-emacs}[nix-emacs-ci] and {uri-evm}[EVM].

==== nix-emacs-ci

Using {uri-nix-emacs}[nix-emacs-ci] is easy: define environment
variable `$EMACS_CI` with the desired Emacs version and `curl` a
single shell script.  With one more line you can also install Eldev.
...
Another (older) tool to install Emacs is {uri-evm}[EVM].  It appears
to be faster than `nix-emacs-ci`.  However, Steve Purcell (the author
of `nix-emacs-ci`) mentions “various issues” he has had with it;
likewise, I also have had some.

Nevertheless, Eldev itself uses EVM at the moment.  If you also want
to try it, Eldev provides a simple script specifically for use on
Travis CI that installs Eldev and EVM in one go.  Here is a simple
project-agnostic `.travis.yml` file that you can use as a basis:
