...
loading mode.  However, in order for the autoloads file to be built
automatically, you need to load them in one of the “built” mode:
`built`, `built-and-compiled` or `built-source`.  (Or `packaged` —
this, of course, also works.)  Otherwise, the autoloads file may be
out-of-date or not generated at all.

So, when adding a local dependency that uses the `autoloads` plugin,
you need to specify it like this:

[source]
----
(eldev-use-local-dependency "~/barlib" 'built)
----
...
If you use a project with this plugin as a local dependency, you will
need to load it in `built`, `built-and-compiled`, `built-source` or
`packaged` mode.  Otherwise file `DEPENDENCY-autoloads.el` may be
out-of-date or not generated at all.
...
[source]
----
(eldev-use-local-dependency "~/mylib" 'built-and-compiled)
----
