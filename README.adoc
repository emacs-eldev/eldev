...
profiling, though they could, of course, be useful in other cases.

Options `--macroexpand` and `--compile` (`-m` and `-c`) make the
commands preprocess their expressions accordingly: by default
expressions are evaluated using Elisp interpreter.
...
result is printed.  However, with `exec` any side effects (including
printing) will be observable that many times, so keep that in mind!
