...
Here is a short list of projects that use Eldev.  I intentionally list
only my own projects, even if there are others, because this way it’s
easier to ensure that the comments below stay valid.  Eldev itself
comes with no examples: I think real-world usage provides better
models.

{uri-extmap}[`extmap`]; its file {uri-extmap}/blob/master/Eldev[`Eldev`]::

    A trivial project with no dependencies.  As you can see, there is
...
{uri-iter2}[`iter2`]; its file {uri-iter2}/blob/master/Eldev[`Eldev`]::

    Another trivial project with no dependencies.  However, it uses
    its file `Eldev` to define a custom option that activates
    project-specific development assistance code.

{uri-logview}[Logview]; its file {uri-logview}/blob/master/Eldev[`Eldev`]::
...
{uri-datetime}[`datetime`]; its file {uri-datetime}/blob/master/Eldev[`Eldev`]::
...
integration>> on <<travis-ci,Travis CI>> for automated testing.
...
archives it knows about priorities 300, 200 and 100 in the order they
are listed above.  A dependency from a package with a lower priority
is installed only if there are no other options.
...
Travis CI is perhaps the most used continuous integration service for
Elisp code, at least until the addition of GitHub actions.  The
largest problem on {uri-travis}[Travis CI] is to install Emacs binary
of the desired version.  Luckily, there are tools that can be used for
this: at least {uri-evm}[EVM] and {uri-nix-emacs}[nix-emacs-ci].
...
with it, however Eldev itself and many other projects use it.
Apparently, you need to fix Ubuntu distribution used at Travis CI to
Trusty for EVM-provided binaries.
...
Emacs version and `curl` a single shell script.  With one more line
you can also install Eldev.  It appears to be slower than EVM, but for
continuous integration that’s not terribly important.
