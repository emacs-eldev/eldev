  ...
  `(let* ((eldev--test-project (or ,test-project eldev--test-project)))
     ...
     (let ((eldev--test-eldev-local (concat ":pa:" (eldev--test-tmp-subdir "eldev-archive-1")))
       ...
       (eldev--test-run nil ("version")
         ...
         (should (= exit-code 0)))
       (eldev--test-run nil ("--setup" `(setf eldev--upgrade-self-from-forced-pa ,(eldev--test-tmp-subdir "eldev-archive-2")) "upgrade-self")
...
(ert-deftest eldev-upgrade-self-2 ()
  ...
  (let ((eldev--test-project     "trivial-project")
    ...
    (eldev--test-run nil ("version")
      (should (string= stdout (format "eldev %s\n" (eldev-message-version (eldev-find-package-descriptor 'eldev)))))
      (should (= exit-code 0)))
    (eldev--test-run nil ("--setup" `(setf eldev--upgrade-self-from-forced-pa ,(eldev--test-tmp-subdir "eldev-archive-1")) "upgrade-self")
...
(ert-deftest eldev-upgrade-self-dry-run-1 ()
  ...
  (let ((eldev--test-project     "trivial-project")
    ...
    (eldev--test-run nil ("version")
      (should (string= stdout (format "eldev %s\n" (eldev-message-version (eldev-find-package-descriptor 'eldev)))))
      (should (= exit-code 0)))
    (eldev--test-run nil ("--setup" `(setf eldev--upgrade-self-from-forced-pa ,(eldev--test-tmp-subdir "eldev-archive-2")) "upgrade-self" "--dry-run")
